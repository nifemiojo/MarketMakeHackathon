...
class User:
    ...
    def __init__(self, userName, password, web3Instance, mainnetInstance):
        self.web3Instance = web3Instance
        self.mainnetInstance = mainnetInstance

    def addNewAccount(self, address, accountName):
        self.accounts.append(Account(address, accountName, self.web3Instance))
    ...
    def getFavouritesPrices(self, contractAddress):
        ...
        address= self.mainnetInstance.toChecksumAddress(contractAddress)
        contract = self.mainnetInstance.eth.contract(address=address, abi=abi)
    ...
    def deposit(self, account, token, amount):
        func = lambda x: x.name == account
        selectedAmount = list(filter(func, self.accounts))
        txHash = selectedAmount[0].depositFundsToAave(token, amount)
